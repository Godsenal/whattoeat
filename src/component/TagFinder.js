...
class TagFinder extends Component {
  ...
  render() {
    ...
    let suggestions =
      getSuggest.tags.length == 0 && isAdd
        ? [{ name: word + '...을(를) 새로운 태그로 추가', value: word }]
        : getSuggest.tags;
    return (
      <div className={cx('tagFinderContainer')}>
        {!isAdd && (
          <div
            className={cx(
              'tagFinderButton',
              isSearch ? 'tagFinderButton-inactive' : null,
            )}
          ...
          >
            <FaPlus
              className={cx(
                'tagFinderButtonIcon',
                isSearch ? 'tagFinderButtonIcon-rotate' : null,
              )}
            />
          </div>
        )}
        ...
        <div
          className={cx(
            'tagFinderInput',
            isSearch || isAdd ? 'tagFinderInput-active' : null,
          )}
        >
          <Autosuggest
            ...
            renderSuggestion={item => (
              <div className={cx('tagSuggenstion')}>{item.name}</div>
            )}
            inputProps={{
              placeholder: '태그 검색',
